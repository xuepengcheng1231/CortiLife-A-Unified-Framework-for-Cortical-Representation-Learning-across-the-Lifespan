...
from Dataset import DataAugmentationForMAE_parcel, SurfaceFolder
...
from losses import SurfCliploss, get_surfclip_names
...
def get_args_parser():
    ...
    # Data
    parser.add_argument('--root_path', default='./data_surfclip', type=str)
    parser.add_argument('--train_image_filename', default='No_CHD_pretrain_hc.npy', type=str)
    parser.add_argument('--train_text_filename', default='No_CHD_text_embeddings_pretrain_hc.npy', type=str)
    parser.add_argument('--val_image_filename', default='No_HCP_test.npy', type=str)
    parser.add_argument('--val_text_filename', default='No_HCP_test_text_embeddings.npy', type=str)
    parser.add_argument('--global_mean', default="/data2/member/SurfClip/data_surfclip/No_CHD_hc_mean.npy", type=str)
    parser.add_argument('--global_std', default="/data2/member/SurfClip/data_surfclip/No_CHD_hc_std.npy", type=str)
    ...
    parser.add_argument('--output-dir', default='./Output_deeper_10/No_ADHD_hc_SurfCLIP', type=str, help='path where to save, empty for no saving')
    ...
    # Model
    parser.add_argument('--model', default='SurfClip_512_deeper_10', type=str)
    ...
    parser.add_argument('--weight_mask', default=0.7, type=float, help='weight of reconstruction loss')
...
def main(args):
    ...
    criterion = SurfCliploss(args).cuda(args.gpu)
    ...
    train_loader, train_sampler, val_loader = Surf_get_Dataloader(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     'optimizer': {k: v.detach().cpu() for k, v in optimizer.state_dict().items()},
        #     'scaler': {k: v.detach().cpu() for k, v in scaler.state_dict().items()},
        #     'best_acc': args.best_acc,
        #     'args': args,
        # }, is_best, args.output_dir, epoch + 1, args.save_freq, args.epochs)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, momentum_schedule, args):
    ...
    metric_names = get_surfclip_names()
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        if not math.isfinite(loss.item()):
            ...
            sys.exit(1)
        del online_inputs,inputs,outputs
        torch.cuda.empty_cache()
...
def validate_zeroshot(val_loader, model, args, ema=False):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            image_features_ = model.encode_image(model.tokenizer(images),ema=ema)
            image_features_ = image_features_ / image_features_.norm(dim=-1, keepdim=True)
            text_features_ = utils.get_model(model).encode_text(target)
        ...
        target = torch.arange(image_features.shape[0]).to(logits_per_image.device)
        acc1, acc5 = accuracy(logits_per_image, target, topk=(5, 10))

        top1.update(acc1.item(), image_features.size(0))
        top5.update(acc5.item(), image_features.size(0))
        torch.cuda.empty_cache()
        # measure elapsed time
        batch_time.update(time.time() - end)
        end = time.time()
