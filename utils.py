...
def _to_cpu_state_dict(module):
    # 避免把CUDA张量写进ckpt
    return {k: v.detach().cpu() for k, v in module.state_dict(keep_vars=False).items()}

def _to_cpu_optimizer_state(optimizer):
    # 把优化器的状态张量也搬到CPU
    state = optimizer.state_dict()
    for st in state.get('state', {}).values():
        for k, v in list(st.items()):
            if torch.is_tensor(v):
                st[k] = v.detach().cpu()
    return state

def _to_cpu_scaler_state(scaler):
...
@torch.no_grad()
def save_on_master_cpu(state, is_best, output_dir, epoch=None, save_freq=5, total_epoch=25):
    ...
    # --- 关键：把需要保存的内容先搬到CPU ---
    model = state.get('state_dict', None)   # 仅用于提取state_dict，不存整模型
    if model is not None:
        state['state_dict'] = _to_cpu_state_dict(model)
        state.pop('model_obj', None)

    if 'optimizer' in state and hasattr(state['optimizer'], 'state_dict'):
        state['optimizer'] = _to_cpu_optimizer_state(state['optimizer'])

    if 'scaler' in state and hasattr(state['scaler'], 'state_dict'):
        state['scaler'] = _to_cpu_scaler_state(state['scaler'])

    # 可选：清一下CUDA缓存，降低保存期间的峰值
    torch.cuda.empty_cache()

    # 关闭zip新格式，减少内存峰值（老格式更省内存）
    torch.save(state, ckpt_path, _use_new_zipfile_serialization=False)

    if epoch is not None and (epoch % save_freq == 0):
...
# def save_on_master(state, is_best, output_dir,epoch=None,save_freq=5,toal_epoch=25):
#     if is_main_process():
#         # ckpt_path = f'{output_dir}/checkpoint.pt'
#         best_path = f'{output_dir}/checkpoint_best_{state["best_acc"]}_{state["best_auc"]}.pt'
#         if is_best:
#             for fname in os.listdir( f'{output_dir}'):
...
def save_on_master(state, is_best, output_dir,epoch=None,save_freq=5,toal_epoch=25):
    if is_main_process():
        ...
        if epoch is not None:
            ...
            if epoch % save_frep == 0:
                each_ckpt_path = f'{output_dir}/checkpoint_{epoch}.pt'
                torch.save(state, each_ckpt_path)
                # shutil.copyfile(ckpt_path, each_ckpt_path)
        if is_best:
            shutil.copyfile(ckpt_path, best_path)
    del state
...
def save_on_master_parcellation(state, is_best, output_dir,epoch=None,save_freq=5,toal_epoch=25):
    if is_main_process():
        # ckpt_path = f'{output_dir}/checkpoint.pt'
        best_path = f'{output_dir}/checkpoint_best_{state["best_dice"]}.pt'
        if is_best:
            for fname in os.listdir( f'{output_dir}'):
                if fname.startswith("checkpoint_best"):
                    fpath = os.path.join(output_dir, fname)
                    ...
                    print(f"Deleted: {fpath}")
        # torch.save(state, ckpt_path)
        np.save(f'{output_dir}/best_perdice.npy', state["per_class_dice"].detach().cpu().numpy())
        # if epoch is not None:
        #     save_frep = save_freq
        #     if epoch % save_frep == 0:
        #         each_ckpt_path = f'{output_dir}/checkpoint_{epoch}.pt'
...
def save_on_master_regression(state, is_best, output_dir,epoch=None,save_freq=5,toal_epoch=25):
    if is_main_process():
        # ckpt_path = f'{output_dir}/checkpoint.pt'
        best_path = f'{output_dir}/checkpoint_best_{state["best_mae"]}.pt'
        if is_best:
            for fname in os.listdir( f'{output_dir}'):
                if fname.startswith("checkpoint_best"):
                    fpath = os.path.join(output_dir, fname)
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos, class_token=False):
    """
