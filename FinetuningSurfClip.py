...
def get_args_parser():
    ...
    parser.add_argument('--finetune', default='/data2/member/SurfClip/Output/No_ADHD_SurfCLIP/checkpoint_25.pt', type=str)
    ...
    parser.add_argument('--output-dir', default='./Output/No_ADHD_SurfCLIP_finetune_25', type=str, help='path where to save, empty for no saving')
...
def Surf_get_finetune_Dataloader(args):
    ...
    label = np.load(label_path).T[:,2].astype(int)
...
def main(args):
    ...
    for iter in range(1):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            acc1 = val_stats['acc1']
            Sensitivity = val_stats['Sensitivity']
            Specificity = val_stats['Specificity']
            AUC = val_stats['ROC_AUC']
            F1 = val_stats['F1_Score']
            is_best = (acc1 > args.best_acc) & (AUC > args.best_auc)
            if is_best:
                args.best_acc = max(acc1, args.best_acc)
                args.best_auc = AUC
                args.best_Sensitivity = Sensitivity
                args.best_Specificity = Specificity
                args.best_F1 = F1
        ...
        best_acc.append(args.best_acc)
        best_sensitivity.append(args.best_Sensitivity)
        best_specificity.append(args.best_Specificity)
        best_F1.append(args.best_F1)
        best_auc.append(args.best_auc)
        np.save(args.output_dir + '/best_acc.npy', best_acc)
        np.save(args.output_dir + '/best_sensitivity.npy', best_sensitivity)
        np.save(args.output_dir + '/best_specificity.npy', best_specificity)
        np.save(args.output_dir + '/best_F1.npy', best_F1)
        np.save(args.output_dir + '/best_auc.npy', best_auc)
    print(best_acc)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        with amp.autocast(enabled=not args.disable_amp):
            ...
            loss = criterion(outputs["cls"], torch.nn.functional.one_hot(online_inputs[1].long(),num_classes=2).float())
            loss_dict = {'loss': loss}
...
def validate_zeroshot(val_loader, model, args, ema=False):
    ...
    with torch.no_grad():
        ...
        logits_per_image = torch.nn.functional.softmax(image_features,dim=-1)
        acc1 = accuracy(logits_per_image, targets, topk=(1,))
        results_stats = calculate_metrics(targets.detach().cpu().numpy(), logits_per_image.detach().cpu().numpy())
        print(acc1)
        top1.update(acc1[0].item(), image_features.size(0))
        batch_time.update(time.time() - end)
        end = time.time()
    ...
    print('0-shot * Acc@1 {top1.avg:.3f}'
          .format(top1=top1))
    return {'acc1': top1.avg, 'Sensitivity': results_stats['Sensitivity'], 'Specificity': results_stats['Specificity'], 'ROC_AUC': results_stats['ROC_AUC'], 'F1_Score': results_stats['F1_Score']}
