...
from Dataset import DataAugmentationForMAE_parcel, SurfaceFolder, Finetune_SurfaceFolder, DataAugmentationForParcel
...
def get_args_parser():
    ...
    parser.add_argument('--save_path', default='/data2/member/SurfClip/data/ADHD', type=str)
    parser.add_argument('--surf_name', default='ADHD.npy', type=str)
    parser.add_argument('--label_name', default='ADHD_label.npy', type=str)
    parser.add_argument('--finetune', default='/data2/member/SurfClip/Output/No_ADHD_SurfCLIP/checkpoint_25.pt',
                        type=str)
    ...
    parser.add_argument('--freeze-depth', default=10, type=int)
    # parser.add_argument('--metadata', default='yfcc15m.pkl', type=str,
    #                 help='path to metadata file (see README for details)')
    parser.add_argument('--root', default='', type=str,
                        help='path to dataset root')
    parser.add_argument('--output-dir', default='./Output/No_ADHD_SurfCLIP_finetune_25', type=str,
    ...
    # Model
    parser.add_argument('--model', default='finetuneSurfClip_512_deeper_6', type=str)
...
def get_model(args):
    ...
    model = getattr(SurfClip, args.model)(global_mean_path=args.global_mean,
                                          ...
                                          Pretrain = True,
                                          device=args.gpu) # note that args.mask_ratio is by default 0
    # model = getattr(models, args.model)()  # note that args.mask_ratio is by default 0
    model.cuda(args.gpu)
...
def Surf_get_Dataloader(args):
    ...
    dataset_train = SurfaceFolder(args.root_path, None, image_path=args.train_image_filename,
                                  text_path=args.train_text_filename)
    dataset_val = SurfaceFolder(args.root_path, None, image_path=args.val_image_filename,
                                text_path=args.val_text_filename)
    if args.distributed:
...
def Surf_get_finetune_Dataloader(args):
    # transform = DataAugmentationForParcel(args)
    # print("Data Aug = %s" % str(transform))
    data_path = os.path.join(args.root_path, args.surf_name)
    label_path = os.path.join(args.root_path, args.label_name)
    data = np.load(data_path)
    label = np.load(label_path).T[:,:].astype(int)
    print(label.shape)
    if args.label_name == "CHD_label.npy":
        label = np.load(label_path)[:, 2].astype(int)
    data = np.concatenate([data[:, :, :40962], data[:, :, 163842:163842 + 40962]], axis=2)
    if args.global_mean != None:
        data = (data - np.load(args.global_mean).reshape([1, 6, 1])) / np.load(args.global_std).reshape([1, 6, 1])
    # if args.model != "finetuning_SurfClip_512_deeper_10_single_channel":
    #     parcel_index = Get_parcellation()
    #     parcel_index = np.concatenate([parcel_index, parcel_index + 40962], axis=0).astype(int)
    #     x_parcel = data[:,:, parcel_index]
    #     print(x_parcel.shape)
    #     data = x_parcel.transpose(0, 1,3,2).reshape([data.shape[0],-1, 640]).transpose(0,2,1)
    ...
    dataset_train = Finetune_SurfaceFolder(args.root_path, None, image_path=args.train_image_filename,
                                           label_path=args.train_label_filename)
    dataset_val = Finetune_SurfaceFolder(args.root_path, None, image_path=args.val_image_filename,
                                         label_path=args.val_label_filename)
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset_train)
        val_sampler = torch.utils.data.distributed.DistributedSampler(dataset_val)
    else:
...
def freeze_visual_layers(model, freeze_depth=10):
...
def main(args):
    ...
    lr_schedule = utils.cosine_scheduler(args.lr, args.lr_end, args.epochs,
                                         len(train_loader) // args.update_freq, warmup_epochs=args.warmup_epochs,
    ...
    for iter in range(1):
        ...
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"Total trainable parameters: {total_params}")
        print([name for name, p in model.named_parameters() if not p.requires_grad])
        if args.output_dir:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if (epoch + 1) % args.eval_freq != 0:
                continue
            val_stats, per_class_dice = validate_zeroshot(val_loader, model, args)
            mean_dice = val_stats['mean_dice']

            is_best = mean_dice > args.best_dice
            if is_best:
                args.best_dice = max(mean_dice, args.best_dice)
                args.best_perdice = per_class_dice

            print("=> saving checkpoint")
            utils.save_on_master_parcellation({
                'epoch': epoch + 1,
        ...
        best_dice.append(args.best_dice)
        np.save(args.output_dir + '/best_dice.npy', best_dice)
    print(best_dice)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        with amp.autocast(enabled=not args.disable_amp):
            ...
            loss = criterion(outputs["cls"],online_inputs[1].long())
            loss_dict = {'loss': loss}
...
@torch.no_grad()
def multiclass_dice(
    logits: torch.Tensor,          # (B, C, V)
    labels: torch.Tensor,          # (B, V)  Long
    num_classes: int = 36,
    ignore_index: int | None = None,
    mode: str = "hard",            # "hard" 或 "soft"
    ignore_empty_label: bool = True,
    eps: float = 1e-6,
):
    """
    返回:
      per_class_dice: (C,)  每一类 Dice
      mean_dice:      ()    宏平均 Dice
    """
    B, C, V = logits.shape
    assert C == num_classes, f"num_classes={num_classes}, but logits has C={C}"

    # 有效位置 mask
    if ignore_index is None:
        valid_mask = torch.ones(B, V, dtype=torch.bool, device=logits.device)
    else:
        valid_mask = labels != ignore_index

    # -------- 生成 one-hot 的 GT --------
    # (B, V) -> (B, C, V)
    gt_oh = F.one_hot(labels.clamp(0, C-1), num_classes=C).permute(0, 2, 1).float()  # (B,C,V)
    # 把无效位置置 0
    gt_oh = gt_oh * valid_mask.unsqueeze(1).float()

    # -------- 预测的 one-hot/prob --------
    if mode == "hard":
        # argmax -> one-hot
        pred_idx = logits.argmax(dim=1)                                     # (B,V)
        pred_oh = F.one_hot(pred_idx, num_classes=C).permute(0, 2, 1).float()  # (B,C,V)
        pred_oh = pred_oh * valid_mask.unsqueeze(1).float()
    elif mode == "soft":
        # softmax 概率（软 Dice）
        prob = F.softmax(logits, dim=1) * valid_mask.unsqueeze(1).float()   # (B,C,V)
        pred_oh = prob
    else:
        raise ValueError("mode must be 'hard' or 'soft'")

    # -------- 逐类 Dice（向量化）--------
    # 交集、预测总量、GT 总量按 (B, C, V) 在 B 与 V 上求和 -> (C,)
    intersection = (pred_oh * gt_oh).sum(dim=(0, 2))              # (C,)
    pred_sum    = pred_oh.sum(dim=(0, 2))                         # (C,)
    gt_sum      = gt_oh.sum(dim=(0, 2))                           # (C,)

    per_class_dice = (2 * intersection + eps) / (pred_sum + gt_sum + eps)  # (C,)

    # -------- 宏平均 --------
    if ignore_empty_label:
        # 仅对 GT 中出现过的类别取平均（避免 GT 完全缺失的类把均值拉低/不稳定）
        valid_cls = gt_sum > 0
        if valid_cls.any():
            mean_dice = per_class_dice[valid_cls].mean()
        else:
            mean_dice = per_class_dice.mean()  # 所有都空时退化处理
    else:
...
def validate_zeroshot(val_loader, model, args, ema=False):
    ...
    with torch.no_grad():
        ...
        logits_per_image = torch.nn.functional.softmax(image_features, dim=-2)
        per_class_dice, mean_dice = multiclass_dice(logits_per_image,targets.long(), num_classes=36)

        if i % args.print_freq == 0:
            progress.display(i)

    # progress.synchronize()
    print('0-shot * Dice@1 {mean_dice:.3f}'
          .format(mean_dice=mean_dice))
    return {'mean_dice': mean_dice.item()}, per_class_dice
...
# CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 --nnodes=1 --master_port=29502 \
# FinetuningParcellationSurfClip.py -j 4 --save_path /data2/member/SurfClip/data/HCP \
# --surf_name HCP_CT.npy --label_name HCP_vertex_label.npy \
# --finetune /data2/member/SurfClip/Output_single/No_HCP_SurfCLIP/checkpoint_10.pt \
# --output-dir ./Output_single/No_HCP_parcellation_CT_SurfCLIP_epoch10_1111 --data-seed 1111 \
# --model Parcellation_finetuning_SurfClip_512_deeper_10_single_channel
