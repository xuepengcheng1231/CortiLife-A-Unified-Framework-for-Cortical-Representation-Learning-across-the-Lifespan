...
def Get_neighs_order_2ring(n_vertex, rotated=0):
    adj_mat_order = sio.loadmat(abspath + '/neigh_indices/adj_mat_order_2ring_' + \
                                str(n_vertex) + '.mat')
...
def downsample_ico(surface_path, ntarget,outsurface_path):
    V0 = 12
    F0 = 20
    vertex, faces = nib.freesurfer.read_geometry(surface_path)
    n = int(np.round(math.log((vertex.shape[0]-2)/(V0-2))/math.log(4)))
    new_vertex = vertex[:4**ntarget*10+2]

    for j in range(n - 1, ntarget - 1, -1):
        print(f"Processing level {j}")
        # Calculate the number of vertices at level j
        nVj = 4 ** j * (V0 - 2) + 2
        # Initialize new faces for this level
        facnew = np.zeros((4 ** j * F0, 3), dtype=int)

        # Find faces that are at the higher subdivision level
        fout = np.where(np.all(faces >= nVj, axis=1))[0]
        for f, face_idx in enumerate(fout):
            # Get the vertex indices for the face
            vidx = faces[face_idx, :]
            # Find the faces that share two vertices with this face
            mask = np.sum(
                (faces == vidx[0]) | (faces == vidx[1]) | (faces == vidx[2]),
                axis=1
            ) == 2
            ftomerge = faces[mask]

            # Compute the new face
            facnew[f, :] = np.sum(ftomerge * (ftomerge < nVj), axis=0)

        # Update faces to the new set of faces for the next iteration
        faces = facnew
    # 创建 PolyData 对象
    mesh = nib.freesurfer.write_geometry(outsurface_path,new_vertex,faces)
    print(". Done.")
    return mesh

def found_target_central_faces(central_face,faces_in_order,neighbor_orders):
    newfaces = []
    print("central:",central_face)
    mask_1 = (np.sum(
            (neighbor_orders == central_face[0]) | (neighbor_orders == central_face[1]) | (
                        neighbor_orders == central_face[2]),
            axis=1
        ) == 2)
        #       | (np.sum(
        #     (neighbor_orders == central_face[0]) | (neighbor_orders == central_face[1]) | (
        ...
        #     axis=1
        # ) == 3))
    print(neighbor_orders[mask_1])
    index = mask_1.nonzero()
    print(index)
    index = np.unique(index[0])
    new_index = []
    for i in index:
        if len(set(neighbor_orders[i])) == len(neighbor_orders[i]):
            new_index.append(i)
        # elif len(set(neighbor_orders[i])) == (len(neighbor_orders[i])-1):
        #     if np.sum((neighbor_orders[i] == central_face[0]) | (neighbor_orders[i] == central_face[1]) | (neighbor_orders[i] == central_face[2])) == 3:
        #         new_index.append(i)

    index = new_index
    print(index)
    mask = np.sum(
        (faces_in_order == index[0]) | (faces_in_order == index[1]) | (
                faces_in_order == index[2]),
        axis=1
    ) == 3
    print(faces_in_order[mask])
    newfaces.append(faces_in_order[mask][0,:])
    ...
    ftomerge = faces_in_order[mask]
    for index in range(ftomerge.shape[0]):
        newfaces.append(ftomerge[index])
    return newfaces

def get_parcellation():
    vertex_6,faces_6 = nib.freesurfer.read_geometry("./surface/lh.white6.downsample")
    vertex_5,faces_5 = nib.freesurfer.read_geometry("./surface/lh.white5.downsample")
    vertex_4,faces_4 = nib.freesurfer.read_geometry("./surface/lh.white4.downsample")
    vertex_3,faces_3 = nib.freesurfer.read_geometry("./surface/lh.white3.downsample")
    vertex_2,faces_2 = nib.freesurfer.read_geometry("./surface/lh.white2.downsample")
    surface,faces = nib.freesurfer.read_geometry("./surface/lh.white1.downsample")
    neighbor_6 = get_neighs_order(40962).reshape([-1,7])
    neighbor_5 = get_neighs_order(10242).reshape([-1,7])
    neighbor_4 = get_neighs_order(2562).reshape([-1,7])
    neighbor_3 = get_neighs_order(642).reshape([-1,7])
    neighbor_2 = get_neighs_order(162).reshape([-1,7])
    final_parcel = [np.empty(shape=[1]) for _ in range(80)]
    for i, face in enumerate(faces):
        central_faces = face.reshape([-1,3])
        for order in range(2,4):
            print("#"*30,order)
            new_central_faces = []
            for central_face_index in range(central_faces.shape[0]):
                print(central_faces[central_face_index])
                new_central_face = found_target_central_faces(central_faces[central_face_index],eval(f"faces_{order}"),eval(f"neighbor_{order}"))
                new_central_faces.append(new_central_face)
            print(new_central_faces)
            central_faces = np.array(new_central_faces).reshape([-1,3])
            print(central_faces.shape)
            final_parcel[i] = np.concatenate([final_parcel[i],np.unique(central_faces)],axis=0)
    for final_parcel_idx in range(len(final_parcel)):
        final_parcel[final_parcel_idx] = final_parcel[final_parcel_idx][1:]
        final_parcel[final_parcel_idx] = np.unique(final_parcel[final_parcel_idx])
    print(np.array(final_parcel).shape)
    np.save("parcelation80_642.npy",final_parcel)
    return final_parcel
...
def Get_parcellation(num_parcellation=0):
    parcel_matric = np.load(abspath+'/neigh_indices/parcelation.npy')
    return parcel_matric

def Get_parcellation80_642(num_parcellation=0):
    parcel_matric = np.load(abspath+'/neigh_indices/parcelation80_642.npy')
    return parcel_matric


if __name__ == '__main__':
    parcel = get_parcellation()
    print(parcel.shape)
