...
class onering_conv_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(mat.shape)
        out_features = mat.view(len(x), 7*self.in_feats)
...
class onering_conv_layer_batch(nn.Module):
    ...
    def __init__(self, in_feats, out_feats,neigh_orders, neigh_indices=None, neigh_weights=None, drop_rate=None):
        ...
        self.weight = nn.Linear(7 * in_feats, out_feats,dtype=torch.float)
        
    def forward(self, x):
        # import pdb
        # pdb.set_trace()

        mat = x[:, self.neigh_orders,:].view(x.shape[0], x.shape[1], 7*self.in_feats)
        #print(mat.shape)
        # import pdb
        # pdb.set_trace()
        out_features = self.weight(mat)
        #out_features = mat[:,:40962,:]
        #print(out_features.shape)
        #out_features = out_features.reshape(-1,1,40962,7)
        return out_features
...
class Get_feature(nn.Module):
    ...
    def __init__(self, in_feats, neigh_orders, neigh_indices=None, neigh_weights=None, drop_rate=None):
...
if __name__ == '__main__':
    ...
    conv = onering_conv_layer_batch(1,1, neigh_orders_163842)
    conv2 = onering_conv_layer_batch(1, 1, neigh_orders_40962)
    get_feature = Get_feature(1,neigh_orders_40962)
    pool = pool_layer_batch(neigh_orders_163842)
    pool2 = pool_layer_batch(neigh_orders_40962)
    upconv2 = upconv_layer_batch(7, 1, upconv_top_index_163842, upconv_down_index_163842)
    upconv = upconv_layer_batch(7, 7, upconv_top_index_40962, upconv_down_index_40962)
    data = torch.rand(10,163842,1)
    tmp1 = conv(data)
    print(tmp1.shape)
    tmp1 = pool(tmp1)
    print(tmp1.shape)
    temp2 = conv2(tmp1)
    print(temp2.shape)
    temp3 = get_feature(temp2)
    print(temp3.shape)
    temp3 = upconv(temp3)
    print(temp3.shape)
    output = upconv2(temp3)
    print(output.shape)
